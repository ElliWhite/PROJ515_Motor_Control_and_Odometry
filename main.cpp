...
int main() {
    ...
    double vx = 0.0;                        // Linear velocity of robot (m/s)
    ...
    while(nh.connected() && killSwitchMonitor == 1){
        ...
        *******************************************************************/
        ...
        mat_wheel_vels = (mat_conversion * mat_target_vels);        // Perform cross-product multiplication

        double right_target_vel = WHEEL_RADIUS * double(mat_wheel_vels.getNumber(1,1)); // Calculate target velocity for right wheel (m/s)
        double left_target_vel = WHEEL_RADIUS * double(mat_wheel_vels.getNumber(2,1));  // Calculate target velocity for left wheel (m/s)
